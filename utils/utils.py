...
class Dataloader(object):
    ...
    def __init__(self, examples, labels, batch_size=1, shuffle=False):
...
def fit(model, examples, labels, nums_epochs, learning_rate=1e-3, batch_size=1, print_info=True, **kw):
    ...
    for epoch in np.arange(nums_epochs):
        for (batch_examples, batch_labels) in dataloader:
            yhat, loss = model.optimize(batch_examples, batch_labels, alpha=learning_rate, **kw)
            losses.append(loss)
